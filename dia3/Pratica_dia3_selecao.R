...
getwd()
setwd("/indicar_caminho_para_diretório_dos_arquivos")

#### D Tajima ####


# Leia os arquivos gerados para o teste D de Tajima no R
AFR_TajimaD<-read.table("AFR_maf.Tajima.D", header=T)
EUR_TajimaD<-read.table("EUR_maf.Tajima.D", header=T)
EAS_TajimaD<-read.table("EAS_maf.Tajima.D", header=T)
...
FST_AFR_EAS <- read.table("AFR_EAS_maf.weir.fst", header=T)
FST_AFR_EUR <- read.table("AFR_EUR_maf.weir.fst", header=T)
FST_EAS_EUR <- read.table("EAS_EUR_maf.weir.fst", header=T)
...
## Em qual gene está o SNP da posição 136608646 do cromossomo 2 (build 37/hg19)?
# Precure a informação no Genome Browser da UCSC: https://genome.ucsc.edu/cgi-bin/hgGateway
...
# Primeira estimar o FST por janela
...
dem_AFR_EUR<-slideFunct(FST_AfrEur_data_clean$DEM,20,15)
dem_EAS_EUR<-slideFunct(FST_EasEur_data_clean$DEN,20,15)


## FST ratio of average
...
# Recuperar janela com o SNP de interesse
...
# Plotar a linha de corte para o quantil 0.95
